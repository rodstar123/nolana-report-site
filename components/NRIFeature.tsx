...
import ScoreBadge, { getNRITier } from "./ScoreBadge";
import NRIHeatmap from "./NRIHeatmap";
import NRITooltip from "./NRITooltip";
import { nriSectionEntrance } from "@/lib/animations";
            ...
            <div className="glass-nolana rounded-xl p-6">
              ...
              <div className="space-y-4">
                {DIMENSIONS.map((dim) => (
                  <div
                    key={dim.name}
                    className="nri-dim-row flex items-start gap-3"
                    style={{ opacity: 0 }}
                  >
                    <div className="nri-dim-icon w-9 h-9 rounded-lg bg-teal/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                      <svg
